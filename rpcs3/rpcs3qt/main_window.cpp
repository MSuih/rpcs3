	...
	else
	{
		...
		if (!cancelled)
		{
			...
			if (result.error == package_install_result::error_type::app_version)
			{
				...
				QString expected = result.version.expected.empty() ? tr("Unknown") : QString::fromStdString(result.version.expected.data());
				QString found = result.version.found.empty() ? tr("Unknown") : QString::fromStdString(result.version.found.data());
				QMessageBox::warning(this, tr("Warning!"), tr("Package cannot be installed on top of the current data.\nUpdate is for version %1, but you have version %2.\n\nTried to install: %3")
						.arg(expected).arg(found).arg(package->path));
			}
			else
			{
